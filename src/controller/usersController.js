...
class UsersController {
  ...
  getUserByCodeName = async (req, res) => {
    ...
    let pool;
    try {
      logger.info(`Intentando obtener el usuario con codeName: ${codeName}`);
      pool = await sql.connect(configBD);

      const request = pool.request();
      request.input('codeName', sql.VarChar, codeName);
      const query = 'SELECT * FROM Users WHERE CodeName = @codeName';
      ...
      logger.error('Error al obtener el usuario CodeName:', error);
      res.status(500).json({ mensaje: 'Error al consultar usuario por CodeName', code: error.code });
    } finally {
      if (pool) {
        await pool.close();
      }
